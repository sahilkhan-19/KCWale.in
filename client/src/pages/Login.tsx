...
export const Login: React.FC = () => {
  ...
  const onInitialSubmit = async (data: LoginFormValues) => {
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await login(data.email, data.password, otp)
        ...
        <Card className="w-full bg-[#18181c]/65 backdrop-blur-xl border border-white/[0.08] text-white shadow-2xl p-4 md:p-5 rounded-[2rem]">
          <CardHeader className="space-y-0.5 text-center pb-2 relative">
            ...
            <img
              src="/logo.png"
              alt="KC Wale Logo"
              className="w-10 h-10 object-contain rounded-xl shadow-sm shadow-black/10 border border-white/10 bg-white mx-auto mb-2"
            />
          ...
          <CardContent className="space-y-3 pb-2 mt-2">
            {error && (
            ...
            {step === 1 ? (
              <form onSubmit={handleSubmit(onInitialSubmit)} className="space-y-3">
                {/* Email Address */}
                <div className="space-y-1">
                  ...
                  <Input
                    ...
                    {...register("email")}
                    className="bg-black/40 border-white/10 focus-visible:ring-primary focus-visible:border-primary text-white placeholder:text-neutral-500 rounded-xl py-3 backdrop-blur-sm h-10"
                  />
                  {errors.email && (
                    <p className="text-xs text-red-500 font-medium ml-1 mt-0.5">{errors.email.message}</p>
                  )}
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <div className="relative">
                    <Input
                      ...
                      placeholder="••••••••"
                      {...register("password")}
                      className="bg-black/40 border-white/10 focus-visible:ring-primary focus-visible:border-primary text-white placeholder:text-neutral-500 rounded-xl py-3 pr-10 backdrop-blur-sm h-10"
                    />
                    <button
                      type="button"
                  ...
                  {errors.password && (
                    <p className="text-xs text-red-500 font-medium ml-1 mt-0.5">{errors.password.message}</p>
                  )}
                </div>
                ...
                <Button
                  ...
                  className="w-full bg-primary hover:bg-primary/95 text-white font-bold py-3 rounded-xl shadow-lg shadow-primary/10 transition-all duration-300 flex items-center justify-center gap-2 cursor-pointer mt-4 h-10"
                >
                  {isSubmitting ? (
                    <>
            ...
            {step === 1 && (
              <>
                ...
                <Button
                  ...
                  className="w-full bg-black/40 hover:bg-black/60 text-white border border-white/10 font-bold py-3 rounded-xl transition-all duration-300 flex items-center justify-center gap-2 cursor-pointer h-10"
                >
                  {isGoogleSubmitting ? (
                    <Loader2 className="w-4 h-4 animate-spin" />
                  ) : (
          ...
          <CardFooter className="flex flex-col gap-1 text-center text-xs text-neutral-500 border-t border-white/[0.04] pt-3 pb-1 mt-2">
            <p>
